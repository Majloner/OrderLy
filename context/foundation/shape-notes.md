...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction + coordination overhead + data trapped in scattered media"
    - topic: "primary persona"
      decision: "venue owner — configures company, menu, room layout, QR codes"
    - topic: "market insight"
      decision: "existing POS/menu systems target chains and are too expensive/heavy for single SMB venues"
    - topic: "staff account provisioning"
      decision: "owner invites/creates staff accounts and assigns roles; no staff self-registration"
    - topic: "role capabilities"
      decision: "owner = superset; config owner-only; waiter = orders + availability; kitchen = kitchen display only"
    - topic: "MVP scope (MVP-too-big surfaced)"
      decision: "partial cut: AI translation deferred to phase 2; ingredients reduced to allergen tags; visual room editor and photo thumbnails kept full; ~7-week timeline accepted"
    - topic: "order model"
      decision: "one open order per table session (running tab); client adds items until staff explicitly closes the order after payment"
    - topic: "order statuses"
      decision: "3 statuses (nowe → w toku → zrealizowane) applied per item, items can be grouped; explicit staff close action ends the session"
    - topic: "domain rule"
      decision: "combination — order lifecycle workflow + availability gate"
  frs_drafted: 19
...
Insight: rynek małych i średnich lokali gastronomicznych jest niedoobsługiwany.
Dostępne systemy POS i menu cyfrowe celują w sieci i większych graczy — są za
drogie i za ciężkie konfiguracyjnie dla pojedynczego lokalu. OrderLY stawia na
jedno, tanie, spójne narzędzie all-in-one: aktywne menu ze zdjęciami, stały kod QR
przy stoliku oraz składanie i obsługa zamówień zarówno przez klienta, jak i personel.
...
przy stoliku składa zamówienia anonimowo po zeskanowaniu kodu QR. MVP służy
przede wszystkim właścicielowi, ale ścieżka klienta z QR jest kluczowym momentem
wartości produktu.
...
**Provisioning:** konta kelnera i kuchni tworzy/zaprasza właściciel i przypisuje
im role. Brak samodzielnej rejestracji personelu.
...
Acknowledged on 2026-05-22: ~7-tygodniowy MVP (przedział 6–8 tyg. pracy po
godzinach) wymaga wytrwałej, sustained dedykacji; użytkownik świadomie zaakceptował
ten koszt po nazwaniu anti-patternu MVP-too-big. Zakres został częściowo ścięty:
tłumaczenie AI menu przeniesione do fazy 2 (MVP jednojęzyczny), pełne definiowanie
składników uproszczone do tagów alergenów (pełne składniki w fazie 2). W pełnej
formie pozostają wizualny edytor schematu sali oraz zdjęcia z generowaniem miniatur.
...
- **Fiskalizacja i integracja z kasami/POS** — brak integracji z urządzeniami
  fiskalnymi; poza zakresem projektu certyfikacyjnego.
...
1. **Kompletność informacji o alergenach (FR-006)** — MVP oznacza alergeny jako
   ...
   Owner: użytkownik.
2. **Dokładna data deadline projektu certyfikacyjnego** — istnieje twardy termin
   wynikający z projektu certyfikacyjnego, ale data nie została jeszcze podana.
   Do uzupełnienia w `timeline_budget.hard_deadline`. Owner: użytkownik.

## Forward: tech-stack

Treści stackowe z dokumentu wejściowego — informacyjne, NIE część schematu PRD;
przeznaczone dla kolejnego kroku łańcucha (10x-tech-stack-selector):

- **Multi-tenancy** — proponowana współdzielona baza danych z kolumną `company_id`
  w każdej tabeli, izolacja danych per firma.
- **Rekomendowany hosting: Supabase** — PostgreSQL z Row Level Security
  (wymuszanie dostępu po `company_id`), wbudowane uwierzytelnianie i role, storage
  na zdjęcia potraw z generowaniem miniatur.
- **Alternatywa (rozdzielone usługi)** — frontend na Vercel/Netlify, backend na
  Railway/Render, zdjęcia na Cloudflare R2 lub AWS S3, PostgreSQL u dowolnego
  dostawcy; większa elastyczność kosztem złożoności konfiguracji.

## Quality cross-check

Cross-check przeprowadzony 2026-05-22. Wszystkie elementy jakości obecne — brak luk:
...
`quality_check_status: accepted`. Otwarte kwestie z `## Open Questions` (alergeny,
data deadline) nie są lukami shape — przepływają do PRD jako Open Questions.

## Forward: technical-roadmap

Propozycje rozszerzeń poza MVP (faza 2+) — informacyjne, NIE część schematu PRD:

- Płatności online oraz podział rachunku między gości przy jednym stoliku.
- Powiadomienia dla personelu i kuchni o nowym zamówieniu z QR (dźwięk/badge w panelu).
- Warianty i dodatki do pozycji (rozmiar, sosy, „bez cebuli").
- Historia zamówień klienta w obrębie sesji stolika.
- Wielojęzyczne menu z tłumaczeniem AI na żądanie + cache w bazie (ścięte z MVP).
- Pełne definiowanie składników pozycji (ścięte z MVP do tagów alergenów).
